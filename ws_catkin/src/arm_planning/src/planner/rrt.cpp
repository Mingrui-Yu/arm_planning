...
bool RRT::solve(
    ...
    while(ros::ok() && iter < rrt_max_iter_){
        ...
        Node::Ptr reached_node = extend(node_list, nearest_node, rand_node);
        ROS_DEBUG_STREAM("solve(): generate reached node.");

        if(req.b_visualize_planning_process_){
            visualizer_->publishText("rand node");
            visualizer_->publishNode(rand_node);
            rate.sleep();

            visualizer_->publishText("nearest node");
            visualizer_->publishNode(nearest_node);
            rate.sleep();

            visualizer_->publishText("reached node");
            visualizer_->publishNode(reached_node);
            rate.sleep();
        }

        if(checkTwoNodeCanConnect(reached_node, goal_node)){
